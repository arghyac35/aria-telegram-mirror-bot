...
if (constants.USE_SERVICE_ACCOUNT && !constants.IS_TEAM_DRIVE) {
  console.log(
    "In order to use Service account for clone the drive should be Team drive. Please set IS_TEAM_DRIVE to true in .constants.js"
  );
  process.exit();
}
...
function setEventCallback(
  ...
  bot.onText(regexpNoName, (msg, match) => {
    ...
    if (msg.chat.type !== "private" && !match[0].match(regexp)) return;
    callback(msg, match);
  });
}
...
setEventCallback(
  eventRegex.commandsRegex.start,
  eventRegex.commandsRegexNoName.start,
  (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
      ...
      msgTools.sendMessage(
        bot,
        msg,
        "You should know the commands already. Happy mirroring.",
        -1
      );
    }
  }
...
setEventCallback(
  eventRegex.commandsRegex.id,
  eventRegex.commandsRegexNoName.id,
  (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      msgTools.sendMessage(
        bot,
        msg,
        "This chat's id is: <code>" + msg.chat.id + "</code>",
        60000
      );
    }
  }
...
setEventCallback(
  eventRegex.commandsRegex.mirrorTar,
  eventRegex.commandsRegexNoName.mirrorTar,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
setEventCallback(
  eventRegex.commandsRegex.mirror,
  eventRegex.commandsRegexNoName.mirror,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
async function checkIfTorrentFile(
  msg: TelegramBot.Message,
  match: RegExpExecArray
) {
  if (
    msg.hasOwnProperty("reply_to_message") &&
    msg.reply_to_message.hasOwnProperty("document") &&
    msg.reply_to_message.document.hasOwnProperty("file_id")
  ) {
    if (
...
setEventCallback(
  eventRegex.commandsRegex.stats,
  eventRegex.commandsRegexNoName.stats,
  async (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      try {
        const diskSpace = await checkDiskSpace(
          constants.ARIA_DOWNLOAD_LOCATION_ROOT
        );
        ...
        msgTools.sendMessage(
          bot,
          msg,
          `Total space: ${downloadUtils.formatSize(
            diskSpace.size
          )}\nUsed: ${downloadUtils.formatSize(
            usedDiskSpace
          )}\nAvailable: ${downloadUtils.formatSize(
            diskSpace.free
        ...
        msgTools.sendMessage(
          bot,
          msg,
          `Error checking stats: ${error.message}`
        );
      }
    }
  }
...
setEventCallback(
  eventRegex.commandsRegex.authorize,
  eventRegex.commandsRegexNoName.authorize,
  async (msg) => {
    if (msgTools.isAuthorized(msg) !== 0) {
      msgTools.sendMessage(bot, msg, `This command is only for SUDO_USERS`);
    } else {
      try {
        let alreadyAuthorizedChats: any = await readFile(
          "./authorizedChats.json",
          "utf8"
        ).catch(async (err) => {
          if (err.code === "ENOENT") {
        ...
        const allAuthorizedChats: number[] = constants.AUTHORIZED_CHATS.concat(
          alreadyAuthorizedChats,
          constants.SUDO_USERS
        );
        if (allAuthorizedChats.includes(msg.chat.id)) {
          ...
          await writeFile(
            "./authorizedChats.json",
            JSON.stringify(alreadyAuthorizedChats)
          ).then(() => {
...
setEventCallback(
  eventRegex.commandsRegex.unauthorize,
  eventRegex.commandsRegexNoName.unauthorize,
  async (msg) => {
    if (msgTools.isAuthorized(msg) !== 0) {
      msgTools.sendMessage(bot, msg, `This command is only for SUDO_USERS`);
    } else {
          ...
          if (index > -1) {
            ...
            await writeFile(
              ...
              msgTools.sendMessage(
                bot,
                msg,
                `Chat unauthorized successfully.`,
                -1
              );
            });
          } else {
            msgTools.sendMessage(
              bot,
              msg,
              `Cannot unauthorize this chat. Please make sure this chat was authorized using /authorize command only.`
            );
          }
        } else {
          msgTools.sendMessage(
            bot,
            msg,
            `No authorized chats found. Please make use this chat was authorized using /authorize command only.`
          );
        }
      } catch (error) {
...
setEventCallback(
  eventRegex.commandsRegex.mf,
  eventRegex.commandsRegexNoName.mf,
  (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      if (
        ...
        bot
          .getFileLink(msg.reply_to_message.document.file_id)
          .then((res) => {
        ...
        msgTools.sendMessage(
          bot,
          msg,
          "Failed to start download. Reply to a torrent file.",
          60000
        );
      }
    }
...
setEventCallback(
  eventRegex.commandsRegex.unzipMirror,
  eventRegex.commandsRegexNoName.unzipMirror,
  (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
setEventCallback(
  eventRegex.commandsRegex.restart,
  eventRegex.commandsRegexNoName.restart,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) !== 0) {
      msgTools.sendMessage(bot, msg, `This command is only for SUDO_USERS`);
    } else {
      try {
        if (!process.env.HEROKU_API_KEY) {
          msgTools.sendMessage(
            bot,
            msg,
            `Can't restart as <code>HEROKU_API_KEY</code> is not provided`
          );
        } else {
          let restartingMsg = await bot.sendMessage(
            msg.chat.id,
            `Heroku dyno will be restarted now.`,
            {
          ...
          await writeFile(
            "./restartObj.json",
            JSON.stringify({ originalMsg: msg, restartingMsg })
          );
          const response = await heroku.delete(
...
setEventCallback(
  eventRegex.commandsRegex.mirrorStatus,
  eventRegex.commandsRegexNoName.mirrorStatus,
  (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
setEventCallback(
  eventRegex.commandsRegex.list,
  eventRegex.commandsRegexNoName.list,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      ...
      driveList.listFiles(match[4], async (err, res) => {
        ...
        if (err) {
          msgTools.sendMessage(
            bot,
            msg,
            "Failed to fetch the list of files: " + err
          );
        } else {
            ...
            try {
              if (res.length === 0) {
                msgTools.sendMessage(
                  bot,
                  msg,
                  "There are no files matching your parameters"
                );
                return;
              }
              ...
              msgTools
                .sendMessageAsync(
                  bot,
                  msg,
                  `Search results for ${match[4]} 👇🏼`,
                  60000,
                  false,
                  [{ buttonName: "Here", url: telegraPhObj.url }]
                )
                .catch(console.error);
            } catch (error) {
              msgTools.sendMessage(
                bot,
                msg,
                "Failed to fetch the list of files, Telegra.ph error: " + error
              );
            }
          } else {
...
setEventCallback(
  eventRegex.commandsRegex.getFolder,
  eventRegex.commandsRegexNoName.getFolder,
  (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      msgTools.sendMessage(
        ...
        msg,
        "<a href = '" +
          driveUtils.getFileLink(constants.GDRIVE_PARENT_DIR_ID, true) +
          "'>Drive mirror folder</a>",
        60000
      );
    }
  }
...
setEventCallback(
  eventRegex.commandsRegex.cancelAll,
  eventRegex.commandsRegexNoName.cancelAll,
  (msg) => {
...
setEventCallback(
  eventRegex.commandsRegex.clone,
  eventRegex.commandsRegexNoName.clone,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
setEventCallback(
  eventRegex.commandsRegex.tar,
  eventRegex.commandsRegexNoName.tar,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
setEventCallback(
  eventRegex.commandsRegex.removeText,
  eventRegex.commandsRegexNoName.removeText,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      ...
      const fileId: string =
        Array.isArray(driveId) && driveId.length > 0 ? driveId[0] : "";
      if (fileId) {
        let cloneMsg = await bot.sendMessage(
          msg.chat.id,
          `Removing text from: <code>` + match[4] + `</code>`,
          {
            reply_to_message_id: msg.message_id,
            parse_mode: "HTML",
          }
        ...
        await removeFn
          .removeText(fileId, matchTxt[1], bot, cloneMsg)
          .then((res: string) => {
...
setEventCallback(
  eventRegex.commandsRegex.count,
  eventRegex.commandsRegexNoName.count,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      ...
      if (fileId) {
        let countMsg = await bot.sendMessage(
          msg.chat.id,
          `Collecting info about ${fileId}，Please wait...`,
          {
            reply_to_message_id: msg.message_id,
            parse_mode: "HTML",
          }
        ...
        const message_updater = async (payload: any) =>
          await msgTools
            .editMessage(bot, countMsg, gen_text(payload))
            .catch((err) => console.error(err.message));

        try {
          let countResult = await gdUtils.gen_count_body({
            fid: fileId,
            tg: message_updater,
          });
          ...
          if (!table) {
            ...
            msgTools.sendMessage(
              bot,
              msg,
              `Failed to obtain info for: ${name}`,
              10000
            );
            return;
          }
          ...
          msgTools
            ...
            .catch(async (err) => {
              if (
                err &&
                ((err.body &&
                  err.body.error_code == 413 &&
                  err.body.description.includes("Entity Too Large")) ||
                  (err.response &&
                    err.response.body &&
                ...
                const limit = 20;
                countResult = await gdUtils.gen_count_body({
                  fid: fileId,
                  limit,
                  smy: countResult.smy,
                });
...
setEventCallback(
  eventRegex.commandsRegex.ytdl,
  eventRegex.commandsRegexNoName.ytdl,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
async function ytdl(msg: TelegramBot.Message, match: RegExpExecArray) {
  try {
    ...
    let ytdlMsg = await bot.sendMessage(
      msg.chat.id,
      `Downloading: <code>` + inputs[0] + `</code>`,
      {
        reply_to_message_id: msg.message_id,
        parse_mode: "HTML",
      }
...
async function clone(msg: TelegramBot.Message, match: RegExpExecArray) {
  ...
  if (fileId) {
    ...
    await cloneFn
      .driveClone(fileId, bot, cloneMsg)
      .then((res: string) => {
        msgTools.deleteMsg(bot, cloneMsg);
        msgTools.sendMessage(bot, msg, res, -1);
      })
...
async function tar(msg: TelegramBot.Message, match: RegExpExecArray) {
  ...
  const fileId: string =
    Array.isArray(driveId) && driveId.length > 0 ? driveId[0] : "";
  if (fileId) {
    const tarMsg = await bot.sendMessage(
      msg.chat.id,
      `Creating Tar: <code>` + match[4] + `</code>`,
      {
        reply_to_message_id: msg.message_id,
        parse_mode: "HTML",
      }
    ...
    driveDownload
      .driveDownloadAndTar(fileId, bot, tarMsg)
      .then((res: string) => {
...
setEventCallback(
  eventRegex.commandsRegex.getLink,
  eventRegex.commandsRegexNoName.getLink,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
      if (constants.INDEX_DOMAIN) {
        ...
        await driveDirectLink
          .getGDindexLink(fileId, true)
          .then((gdIndex: { url: string; name: string }) => {
            let res =
              'Direct Shareable Link: <a href = "' +
              gdIndex.url +
              '">' +
              gdIndex.name +
              "</a>";
...
setEventCallback(
  eventRegex.commandsRegex.help,
  eventRegex.commandsRegexNoName.help,
  async (msg, match) => {
    if (msgTools.isAuthorized(msg) < 0) {
      msgTools.sendUnauthorizedMessage(bot, msg);
    } else {
...
function sendCancelledMessages(): void {
  dlManager.forEachCancelledChat((usernames, tgChat) => {
    var message = usernames.reduce(
      (prev, cur, i) => (i > 0 ? `${prev}${cur}, ` : `${cur}, `),
      usernames[0]
    );
    message += "your downloads have been manually cancelled.";
    bot
      .sendMessage(tgChat, message, { parse_mode: "HTML" })
      .then(() => {
  ...
  if (dlDetails.isUploading || dlDetails.isExtracting) {
    if (cancelMsg) {
      msgTools.sendMessage(
        bot,
        cancelMsg,
        "Upload in progress. Cannot cancel."
      );
    }
    return false;
  } else {
 ...
 *                    or if undetermined
 */
function handleDisallowedFilename(
  dlDetails: details.DlVars,
  filename: string
): boolean {
  if (dlDetails) {
    ...
    .then((res) => {
      ...
      if (res.singleStatuses) {
        res.singleStatuses.forEach(async (status) => {
          if (status.dlDetails) {
            ...
            await driveList
              .isDuplicateMirror(status.filename, status.dlDetails)
              .catch(console.log);
          }
        });
      }
      ...
      dlManager.forEachStatus((status) => {
        ...
        if (res.message !== status.lastStatus) {
          msgTools
            .editMessage(bot, status.msg, res.message, staleStatusReply)
            .catch((err) => {
...
function cleanupDownload(
  ...
  if (dlDetails) {
    ...
    if (!wasCancelAlled) {
      ...
      msgTools
        .sendMessageReplyOriginal(bot, dlDetails, message)
        .catch((err) => {
  ...
  if (dlDetails) {
    ...
    ariaTools.getStatus(dlDetails, async (err, message, filename) => {
      if (!err) {
        ...
        await driveList
          .isDuplicateMirror(filename, dlDetails)
          .catch(console.log);
      }
    });
    ...
    console.error(
      `onDownloadStart: DlDetails still empty for ${gid}. Giving up.`
    );
  }
}
...
function ariaOnDownloadStop(gid: string, retry: number): void {
  ...
  if (dlDetails) {
    ...
    console.error(
      `onDownloadStop: DlDetails still empty for ${gid}. Giving up.`
    );
  }
}
...
function ariaOnDownloadComplete(gid: string, retry: number): void {
  ...
  if (dlDetails) {
    ariaTools.getAriaFilePath(gid, (err, file) => {
      if (err) {
        console.error(
          `onDownloadComplete: Error getting file path for ${gid}. ${err}`
        );
      ...
      if (file) {
        ariaTools.getFileSize(gid, async (err, size) => {
          if (err) {
            console.error(
              `onDownloadComplete: Error getting file size for ${gid}. ${err}`
            );
          ...
          if (handleDisallowedFilename(dlDetails, filename)) {
            const duplicateMirror = await driveList
              .isDuplicateMirror(filename, dlDetails)
              .catch(console.log);
            if (duplicateMirror) {
            ...
            if (dlDetails.isUnzip) {
              try {
                isUnzip = true;
                const extractDetails = await ariaTools.extractFile(
                  dlDetails,
                  file,
                  size
                );
            ...
            console.log(
              `${gid}: Completed. Filename: ${filename}. Starting upload.`
            );
            ariaTools.uploadFile(
              dlDetails,
              file,
              size,
              isUnzip,
              driveUploadCompleteCallback
            );
          } else {
        ...
        ariaTools.isDownloadMetadata(gid, (err, isMetadata, newGid) => {
          if (err) {
            console.error(
              `${gid}: onDownloadComplete: Failed to check if it was a metadata download: ${err}`
            );
            var message =
              "Upload failed. Could not check if the file is metadata.";
            cleanupDownload(gid, message);
          } else if (isMetadata) {
    ...
    console.error(
      `${gid}: onDownloadComplete: DlDetails still empty. Giving up.`
    );
  }
}
  ...
  if (dlDetails) {
    ariaTools.getError(gid, (err, res) => {
      ...
      if (err) {
        ...
        console.error(
          `${gid}: failed. Failed to get the error message. ${err}`
        );
      } else {
...
function driveUploadCompleteCallback(
  err: string,
  gid: string,
  url: string,
  filePath: string,
