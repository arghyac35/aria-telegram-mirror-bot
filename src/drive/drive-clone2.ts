                    ...
                    try {
                        ...
                        gdrive.getSharableLink(copiedFolder.id, true, (err, url) => {
                            ...
                            if (constants.INDEX_DOMAIN) {
                                msg += `\n\n<a href="` + dlUtils.checkTrailingSlash(constants.INDEX_DOMAIN) + 'GdriveBot/' + encodeURIComponent(meta.data.name) + `/">Index URL</a>`
                            }
                    ...
                    await copy_file(meta.data.id, constants.GDRIVE_PARENT_DIR_ID).then((new_file: any) => {
                        if (new_file) {
                            ...
                            gdrive.getSharableLink(new_file.id, false, (err, url) => {
                                ...
                                if (constants.INDEX_DOMAIN) {
                                    msg += `\n\n<a href="` + dlUtils.checkTrailingSlash(constants.INDEX_DOMAIN) + 'GdriveBot/' + encodeURIComponent(new_file.name) + `">Index URL</a>`
                                }
                                new_file.url = url;
